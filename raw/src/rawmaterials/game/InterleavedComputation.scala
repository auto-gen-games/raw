 ...
 *
 * @param currentResult The running result before the remaining steps are performed
 ...
 * @param averageTimePerUnit The average time taken per computation step unit size (starts at 0.0 by default and then updated automatically)
 * @param sizeCompleted The number of computation step units completed so far (starts at 0 by default and updated automatically)
 * @tparam ReferenceData Type of the static data made available to every step in the computation
 * @tparam ResultModel Type of the interleaved computation's (partial/complete) result
 */
case class InterleavedComputation[ReferenceData, ResultModel] (
  ...
  /**
   * Perform all the remaining steps without interleaving.
   * @return The final state of the completed computation
   */
  def performAll: InterleavedComputation[ReferenceData, ResultModel] =
    if (isCompleted) this
    else performStep.performAll
}
